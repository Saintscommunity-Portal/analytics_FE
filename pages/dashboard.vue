...
const sections = [
  {
    key: "people",
    label: "People",
    subtitle: "Workers, members, age and status mix",
    endpoint: "/people-dashboard",
    route: "/workers",
  },
  {
    key: "prayer",
    label: "Prayer",
    subtitle: "Prayer group and vigil attendance",
    endpoint: "/prayer-dashboard",
    route: "/prayer-group",
  },
  {
    key: "study",
    label: "Study Group",
    subtitle: "Submissions, defaulted work and grading",
    endpoint: "/study-group-dashboard",
    route: "/study-group",
  },
  {
    key: "meetings",
    label: "Meetings",
    subtitle: "Church meeting attendance and offerings",
    endpoint: "/church-meeting-dashboard",
    route: "/church-meetings",
  },
  {
    key: "outreach",
    label: "Outreach",
    subtitle: "Evangelism reports and outcomes",
    endpoint: "/outreach-dashboard",
    route: "/outreaches",
  },
  {
    key: "followups",
    label: "Follow Ups",
    subtitle: "Follow-up activity and worker coverage",
    endpoint: "/followup-dashboard",
    route: "/followups",
  },
];

const entities = computed(
  () => authStore.entities || authStore.admin?.entities || {},
);
const entityChurches = computed(() => {
  const churches =
    entities.value.churches || authStore.admin?.entities?.churches || [];
  return Array.isArray(churches) ? churches : [];
});
const churchOptions = computed(() =>
  entityChurches.value.map((church) => ({
    label: church.label || church.name || `Church ${church.id}`,
    value: church.value || church.id,
    id: church.id || church.value,
    fellowships: church.fellowships || [],
  })),
);
const fellowshipOptions = computed(() => {
  const directFellowships =
    entities.value.fellowships || authStore.admin?.entities?.fellowships;

  if (Array.isArray(directFellowships)) {
    return directFellowships.map((item) => ({
      label: item.label || item.name || `Fellowship ${item.id}`,
...
const cellOptions = computed(() => {
  const directCells = entities.value.cells || authStore.admin?.entities?.cells;

  if (Array.isArray(directCells)) {
    return directCells.map((item) => ({
      label: item.label || item.name || `Cell ${item.id}`,
      value: item.value || item.id,
      id: item.id || item.value,
    }));
  }
  ...
  const selectedFellowship = fellowshipOptions.value.find(
    (item) => Number(item.value) === Number(filters.fellowship_id),
  );
  const source = selectedFellowship
    ? [selectedFellowship]
    : fellowshipOptions.value;
  return source.flatMap((fellowship) =>
    (fellowship.cells || []).map((cell) => ({
      label: cell.label || cell.name || `Cell ${cell.id}`,
      value: cell.value || cell.id,
      id: cell.id || cell.value,
    })),
  );
});
...
const overviewCards = computed(() => [
  {
    label: "Workers",
    value: peopleTotals.value.workers || 0,
    detail: "Total workers",
  },
  {
    label: "Members",
    value: peopleTotals.value.members || 0,
    detail: "Total members",
  },
  {
    label: "Members added in period",
    value: peopleTotals.value.membersAddedInPeriod || 0,
    detail: "Selected period",
  },
  {
    label: "Children",
    value: peopleTotals.value.children || 0,
    detail: "Member records",
  },
]);
const meetingCurrency = computed(() => activeTotals.value.currency || "NGN");
const activeStatRows = computed(() => {
  if (activeSection.value !== "meetings") {
    return Object.entries(activeTotals.value).map(([key, value]) => ({
      label: String(key)
        .replace(/_/g, " ")
        .replace(/([A-Z])/g, " $1"),
      value: value ?? 0,
  ...
  return [
    {
      label: "Attendance reported by workers",
      value: activeTotals.value.total_reported_attendance || 0,
    },
    {
      label: "Attendance reported by ushers",
      value: activeTotals.value.total_physical_attendance || 0,
    },
    {
      label: "Adult attendance",
      value: activeTotals.value.reported_adult_attendance_count || 0,
    },
    {
      label: "Children attendance",
      value: activeTotals.value.reported_children_attendance_count || 0,
    },
    {
      label: "Workers who were present",
      value: activeTotals.value.worker_present_count || 0,
    },
    {
      label: "Workers who were absent",
      value: activeTotals.value.worker_absent_count || 0,
    },
    {
      label: "Members who were present",
      value: activeTotals.value.member_present_count || 0,
    },
    {
      label: "Members who were absent",
      value: activeTotals.value.member_absent_count || 0,
    },
    {
      label: "Average church attendance",
      value: Math.round(
        Number(activeTotals.value.average_church_attendance || 0),
      ),
    },
    {
      label: "First Timers",
      value: activeTotals.value.first_timers_count || 0,
    },
    {
      label: "Offering given",
      value: `${meetingCurrency.value} ${formatAmount(activeTotals.value.total_offering_amount)}`,
    },
  ];
});
...
const peopleAgeData = computed(() => ({
  ...
  datasets: [
    {
      label: "Members",
      data: (people.value.memberAgeDistribution || []).map(
        (item) => item.count,
      ),
...
const peopleStatusData = computed(() => ({
  ...
  datasets: [
    {
      data: (people.value.workersByStatus || []).map((item) => item.count),
      backgroundColor: paletteFor(people.value.workersByStatus || []),
    },
  ],
}));
...
const meetingData = computed(() => ({
  labels: ["Reported", "Physical", "Adult", "Children", "First timers"],
  datasets: [
    {
      label: "Meetings",
      data: [
        ...
        activeTotals.value.reported_adult_attendance_count || 0,
        activeTotals.value.reported_children_attendance_count || 0,
        activeTotals.value.first_timers_count || 0,
      ],
...
const activeChartType = computed(() =>
  activeSection.value === "people" ? "doughnut" : "bar",
);

const timelineData = computed(() => {
  const daily =
    activeDashboard.value?.daily ||
    activeDashboard.value?.participationDaily ||
    [];
  const labels = daily.map((row) => row.date || row.week || row.month);
  const values = daily.map((row) =>
    Number(
      row.total_present ||
        row.total_submitted ||
        row.total_reported_attendance ||
        row.total_outreaches ||
        row.total_followup_activities ||
        row.total_reports_sent ||
        row.count ||
        0,
    ),
  );
  ...
  return {
    ...
    datasets: [
      {
        label: activeConfig.value.label,
        data: values,
        borderColor: "#a83632",
...
async function fetchOverview(force = false) {
  ...
  try {
    ...
    errorMessage.value =
      error?.data?.message || error?.message || "Unable to load dashboard.";
  } finally {
    loading.value = false;
  }
}
  ...
  <section class="space-y-6">
    <div class="flex flex-col justify-between gap-4 md:flex-row md:items-end">
      <div>
        ...
        <h1
          class="m-0 text-3xl font-semibold tracking-tight text-gray-950 dark:text-white"
        >
          Activity Dashboard
        </h1>
        <p
        ...
        >
          A birdseye view of church activities
        </p>
      </div>
      <Button
        label="Refresh"
    ...
    <Card
      class="border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900"
    >
      <template #content>
    ...
    <div class="grid gap-4 md:grid-cols-4">
      ...
      >
        <template #content>
          ...
          <h2
            class="m-0 mt-2 text-2xl font-semibold text-gray-950 dark:text-white"
          >
            {{ card.value }}
          </h2>
      ...
      <button
        ...
        :class="
          activeSection === section.key
            ? 'border-[#a83632] bg-[#a83632]/10 text-[#a83632] dark:bg-[#a83632]/20'
            : 'border-gray-200 bg-white text-gray-700 hover:border-[#a83632]/40 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-200'
        "
    ...
    <Message
      v-if="sectionErrors[activeSection]"
      severity="error"
      :closable="false"
      >{{ sectionErrors[activeSection] }}</Message
    >
      ...
      >
        <template #content>
          <div class="mb-4 flex items-start justify-between gap-3">
            <div>
              <h2
                class="m-0 text-base font-semibold text-gray-950 dark:text-white"
              >
                {{ activeConfig.label }}
              </h2>
            ...
            <NuxtLink
              :to="activeConfig.route"
              class="text-sm font-semibold text-[#a83632] no-underline"
              >View full report</NuxtLink
            >
          </div>
          <Skeleton
            v-if="sectionLoading[activeSection]"
            height="20rem"
            border-radius="16px"
          />
          <div v-else class="h-80">
            <Doughnut
              v-if="activeChartType === 'doughnut'"
      ...
      >
        <template #content>
          ...
          <div class="mt-4 space-y-3">
            <div
              v-for="row in activeStatRows"
              :key="row.label"
              class="flex items-center justify-between gap-4 rounded-xl border border-gray-100 px-3 py-2 text-sm dark:border-gray-800"
            >
              <span class="capitalize text-gray-500 dark:text-gray-400">{{
                row.label
              }}</span>
    ...
    >
      <template #content>
        ...
        <p class="m-0 mt-1 text-sm text-gray-500 dark:text-gray-400">
          Loaded only for the selected section.
        </p>
        <div class="mt-4 h-72">
