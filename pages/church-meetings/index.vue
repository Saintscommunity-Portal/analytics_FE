...
const meetingTypesLoading = ref(false);
const statusOptions = ["pending", "completed"].map((value) => ({
  label: displayValue(value),
  value,
}));
const role = computed(() => authStore.admin?.role);
const creatableMeetingTypes = computed(() =>
  meetingTypes.value.filter((type) => canCreateMeetingType(type.role)),
);
const selectedMeetingType = computed(() =>
  creatableMeetingTypes.value.find((type) => type.value === form.type),
);
const isCustomMeetingType = computed(() => form.type === "custom_meeting");
const isFellowshipScoped = computed(() =>
  ["fellowship_pastor", "fellowship_leader"].includes(role.value),
);
...
const fellowshipOptions = computed(() => {
  ...
  const selectedChurch = churchOptions.value.find(
    (church) =>
      Number(church.value) === Number(filters.church_id || form.church_id),
  );
...
const cellOptions = computed(() => {
  ...
  const selectedFellowship = fellowshipOptions.value.find(
    (fellowship) =>
      Number(fellowship.value) ===
      Number(filters.fellowship_id || form.fellowship_id),
  );
...
const dashboardCurrency = computed(() => totals.value.currency || "NGN");
const statCards = computed(() => [
  {
    label: "Total attendance reported by workers",
    value: formatNumber(totals.value.total_reported_attendance),
  },
  {
    label: "Total attendance reported by ushers",
    value: formatNumber(totals.value.total_physical_attendance),
  },
  {
    label: "Total adults",
    value: formatNumber(totals.value.reported_adult_attendance_count),
  },
  {
    label: "Total children",
    value: formatNumber(totals.value.reported_children_attendance_count),
  },
  {
    label: "Present workers",
    value: formatNumber(totals.value.worker_present_count),
  },
  {
    label: "Absent workers",
    value: formatNumber(totals.value.worker_absent_count),
  },
  {
    label: "Present Members ",
    value: formatNumber(totals.value.member_present_count),
  },
  {
    label: "Absent Members ",
    value: formatNumber(totals.value.member_absent_count),
  },
  {
    label: "Average church attendance",
    value: formatNumber(
      Math.round(Number(totals.value.average_church_attendance || 0)),
    ),
  },
  {
    label: "First Timers",
    value: formatNumber(totals.value.first_timers_count),
  },
]);
  ...
  datasets: [
    {
      ...
      data: [
        Number(totals.value.reported_adult_attendance_count || 0),
        Number(totals.value.reported_children_attendance_count || 0),
  ...
  datasets: [
    {
      ...
      data: meetingTypeRows.value.map((row) =>
        Number(row.total_reported_attendance || 0),
      ),
...
function meetingDateLabel(value) {
  ...
  return `${weekday} ${ordinalDay(date.getDate())} ${monthAndYear}`;
}

function syncMeetingName() {
  if (isCustomMeetingType.value) return;

  const typeLabel = selectedMeetingType.value?.label || "";
  const dateLabel = meetingDateLabel(form.meeting_date);
  form.meeting_name =
...
const dailyAttendanceChartData = computed(() => ({
  ...
  datasets: [
    ...
    },
    {
      label: "Physical",
      data: dailyRows.value.map((row) =>
        Number(row.total_physical_attendance || 0),
      ),
      borderColor: "#2563eb",
...
function canCreateMeetingType(typeRole) {
  const allowed = {
    cell_leader: [
      "admin",
      "pastor",
    ...
    church_pastor: ["admin", "pastor", "church_pastor"],
  };

  return (
    allowed[typeRole || "church_pastor"] || allowed.church_pastor
  ).includes(role.value);
}
...
function meetingActionLabel(meeting) {
  ...
  return canManageMeetingScope(meeting)
...
async function fetchMeetings(page = 1) {
  ...
  try {
    const [list, stats] = await Promise.all([
      ...
      request("/church-meeting-dashboard", {
        method: "GET",
        query: query(page),
      }),
    ]);
...
async function fetchMeetingTypes() {
  meetingTypesLoading.value = true;
  ...
  try {
    ...
    meetingTypes.value = Array.isArray(response?.data)
      ? response.data.map((type) => ({
          label: type.label || type.name || displayValue(type.value),
          value: type.value,
          role: type.role || "church_pastor",
        }))
      : [];

    if (!form.type && creatableMeetingTypes.value.length) {
      form.type = creatableMeetingTypes.value[0].value;
    }
  } catch {
    meetingTypes.value = [];
  } finally {
    meetingTypesLoading.value = false;
  }
}
...
async function fetchParticipantSummary(page = 1) {
  ...
  try {
    ...
    errorMessage.value =
      error?.data?.message ||
      error?.message ||
      "Unable to load church meeting participant summary.";
  } finally {
...
function openCreate() {
  ...
  form.fellowship_id =
    isFellowshipScoped.value || isCellScoped.value
      ? fellowshipOptions.value[0]?.value ||
        authStore.admin?.fellowship_id ||
        null
      : null;
...
async function createMeeting() {
  ...
  try {
    ...
    formError.value =
      error?.data?.message || error?.message || "Unable to create meeting.";
  } finally {
    saving.value = false;
  }
}
...
async function completeMeeting() {
  ...
  try {
    await request(`/church-meetings/${selectedMeeting.value.id}/complete`, {
      method: "PATCH",
    });
    ...
    errorMessage.value =
      error?.data?.message || error?.message || "Unable to complete meeting.";
  } finally {
    saving.value = false;
  }
}
  ...
  <section class="space-y-5">
    ...
    >
      <div>
        ...
        <p class="m-0 mt-2 text-sm text-gray-500">
          Create meetings, complete them, record offerings, and manage report
          counts.
        </p>
      </div>
    ...
    <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
      ...
      >
        <template #content>
          ...
          <h2 class="m-0 mt-2 text-2xl font-semibold text-gray-950">
            {{ card.value }}
          </h2>
        </template>
      </Card>
    ...
    <Card class="border border-gray-200 shadow-sm">
      <template #content>
        <DataTable
          ...
          @page="fetchMeetings($event.page + 1)"
        >
          ...
          <Column field="meetingDate" header="Date"
            ><template #body="{ data }">{{
              displayDate(data.meetingDate)
            }}</template></Column
          >
    ...
    >
      ...
      <div class="grid gap-4">
        <InputText
          ...
          :disabled="!isCustomMeetingType"
        />
        <Select
          v-model="form.type"
          :options="creatableMeetingTypes"
          option-label="label"
          option-value="value"
          placeholder="Type"
          :loading="meetingTypesLoading"
    ...
    >
      <div class="space-y-4">
        <div class="rounded-xl border border-gray-200 bg-gray-50 p-4">
          ...
          <div class="mt-3 grid gap-3 md:grid-cols-4">
            ...
            <div class="space-y-2">
              ...
              >
              <Select
                v-model="participantSummaryFilters.type"
                :options="meetingTypes"
                option-label="label"
                option-value="value"
          ...
          >
            <div class="space-y-2">
              ...
              <InputNumber
                ...
                :min="0"
                :max="participantSummaryInfo.heldMeetings || undefined"
                show-buttons
                class="w-full"
                input-class="w-full"
              />
        ...
        >
          Participation count is based on present attendance records in the
          selected range. Example: <strong>2/4</strong> means a person was
          present for 2 out of 4 meetings held.
        </div>
    ...
    <Dialog
      v-model:visible="completeWarningOpen"
      modal
      header="Complete this meeting?"
      class="w-[92vw] max-w-md"
    >
      <div class="space-y-3">
        <Message severity="warn" :closable="false">
          Completing this meeting will mark it as completed and generate
          attendance records for expected participants.
        </Message>
        <div class="rounded-xl border border-gray-200 bg-gray-50 p-3">
          ...
          <p class="m-0 mt-1 text-xs text-gray-500">
            {{ displayDate(selectedMeeting?.meetingDate) }}
          </p>
        </div>
      </div>
      <template #footer>
        <Button
          ...
          outlined
          :disabled="saving"
          @click="completeWarningOpen = false"
        />
        <Button
          label="Proceed"
          :loading="saving"
...
:global(.dark)
  :deep(
    .church-meetings-table .p-datatable-tbody > tr > td span:not(.p-tag-label)
  ),
...
:global(.dark)
  :deep(.church-meetings-table .p-datatable-tbody > tr > td strong) {
