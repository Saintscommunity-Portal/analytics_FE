...
const fellowshipOptions = computed(() => {
  if (authStore.entities?.fellowships?.length) {
    return authStore.entities.fellowships.map((fellowship) => ({
      label: fellowship.name,
      value: fellowship.id,
      cells: fellowship.cells || [],
    }));
  }
  return churchOptions.value.flatMap((church) =>
...
const cellOptions = computed(() => {
  if (authStore.entities?.cells?.length) {
    return authStore.entities.cells.map((cell) => ({
      label: cell.name,
      value: cell.id,
    }));
  }
  const selected = fellowshipOptions.value.find(
    (fellowship) => Number(fellowship.value) === Number(filters.fellowship_id),
  );
...
const followupChartData = computed(() => ({
  labels: ["No follow-up", "Has been followed up"],
  datasets: [
    {
      data: [
...
async function fetchWorkerSummary(page = 1) {
  ...
  try {
    ...
    workerSummaryRows.value = Array.isArray(response?.data)
      ? response.data
      : [];
    workerSummaryMeta.value = {
      ...workerSummaryMeta.value,
      ...(response?.meta || {}),
    };
    workerSummaryInfo.value = {
      ...workerSummaryInfo.value,
      ...(response?.summary || {}),
    };
    workerSummaryRowsPerPage.value = Number(
      workerSummaryMeta.value.per_page || workerSummaryRowsPerPage.value,
    );
    workerSummaryFirst.value =
      ((Number(workerSummaryMeta.value.current_page) || page) - 1) *
      workerSummaryRowsPerPage.value;
  } catch (error) {
    workerSummaryRows.value = [];
    errorMessage.value =
      error?.data?.message ||
      error?.message ||
      "Unable to load outreach worker summary.";
  } finally {
  ...
  <section class="space-y-5">
    ...
    >
      <div>
        ...
        <p class="m-0 mt-2 text-sm text-gray-500">
          Review worker outreach activities and reports within your scope.
        </p>
      </div>
      <Button
    ...
    <div class="grid gap-4 md:grid-cols-4">
      <Card class="border border-gray-200 shadow-sm"
        ><template #content
          ><p class="m-0 text-sm text-gray-500">Outreaches</p>
          <h2 class="m-0 mt-2 text-2xl font-semibold text-gray-950">
            {{ totals.total_outreaches || 0 }}
          </h2></template
        ></Card
      >
      <Card class="border border-gray-200 shadow-sm"
        ><template #content
          ><p class="m-0 text-sm text-gray-500">Reports</p>
      ...
      <Card class="border border-gray-200 shadow-sm"
        ><template #content
          ><p class="m-0 text-sm text-gray-500">Saved</p>
          <h2 class="m-0 mt-2 text-2xl font-semibold text-green-700">
      ...
      <Card class="border border-gray-200 shadow-sm"
        ><template #content
          ><p class="m-0 text-sm text-gray-500">Followed up</p>
          <h2 class="m-0 mt-2 text-2xl font-semibold text-gray-950">
            {{ totals.has_followup_count || 0 }}
          </h2></template
        ></Card
      >
    </div>
    ...
    <Card class="border border-gray-200 bg-white shadow-sm">
      <template #content>
        <DataTable
          ...
          row-hover
          @page="fetchOutreaches($event.page + 1)"
        >
          ...
          <Column field="fellowshipName" header="Fellowship"
            ><template #body="{ data }">{{
              displayValue(data.fellowshipName)
            }}</template></Column
          >
          <Column field="cellName" header="Cell"
            ><template #body="{ data }">{{
              displayValue(data.cellName)
            }}</template></Column
          >
    ...
    >
      <div class="space-y-4">
        <div class="rounded-xl border border-gray-200 bg-gray-50 p-4">
          <div class="grid gap-3 md:grid-cols-4">
            <div class="space-y-2">
              ...
              <InputText
                v-model="workerSummaryFilters.from_date"
                type="date"
                class="w-full"
              />
            </div>
            <div class="space-y-2">
              ...
              <InputText
                v-model="workerSummaryFilters.to_date"
                type="date"
                class="w-full"
              />
            </div>
            <div class="space-y-2">
              ...
              >
              <Select
                v-model="workerSummaryFilters.report_filter"
          ...
          <div class="mt-3 grid gap-3 md:grid-cols-4">
            <div class="space-y-2">
              ...
              <Select
                v-model="workerSummaryFilters.church_id"
                :options="churchOptions"
                option-label="label"
                option-value="value"
                placeholder="Church"
            ...
            <div class="space-y-2">
              ...
              >
              <Select
                v-model="workerSummaryFilters.fellowship_id"
                :options="fellowshipOptions"
                option-label="label"
                option-value="value"
                placeholder="Fellowship"
            ...
            <div class="space-y-2">
              ...
              <Select
                v-model="workerSummaryFilters.cell_id"
                :options="cellOptions"
                option-label="label"
                option-value="value"
                placeholder="Cell"
            ...
            <div class="space-y-2">
              ...
              <Select
                v-model="workerSummaryFilters.worker_id"
                :options="workerOptions"
                :loading="workersLoading"
                option-label="label"
                option-value="value"
          ...
          <div class="mt-3 flex flex-wrap justify-end gap-2">
            <Button
              ...
              severity="secondary"
              outlined
              @click="clearWorkerSummaryFilters"
            />
            <Button
              label="Apply"
              icon="pi pi-filter"
              ...
              @click="applyWorkerSummaryFilters"
            />
          </div>
        </div>
        ...
        >
          Outreach activity is counted from outreach records. Souls reached is
          counted from outreach reports and respects the selected outcome
          filter.
        </div>
        ...
        >
          <Column field="workerName" header="Worker" style="min-width: 210px">
            <template #body="{ data }">
              <div>
                <p class="m-0 font-semibold text-gray-950">
                  {{ data.workerName }}
                </p>
          ...
          <Column
            field="averageParticipationPerWeek"
            header="Avg / week"
            style="min-width: 120px"
          />
          <Column
            field="soulsReachedCount"
            header="Souls reached"
...
:global(.dark)
  :deep(.outreaches-table .p-datatable-tbody > tr > td span:not(.p-tag-label)),
